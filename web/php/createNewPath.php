<?php
	...
	try {
		...
		$myItemId = isset($argv[2]) ? $argv[2] : null;
		if (empty($project_id)) {
			die("");
		}
		...
		foreach($projects as $project) {
			...
			foreach($items as $item) {
			   $destFloorId = (string)$item->xpath('parent::*')[0]->xpath('parent::*')[0]['id'];
			   $id = $item['id'];
			   $itemId = $item->item['id'];
			   ...
			   foreach($terminals as $terminal) {
				...
				$term = $term[0]->xpath( 'parent::*' )[0];
				$floorId = (string)$term[0]->xpath( 'parent::*' )[0]->xpath('parent::*')[0]['id'];
				...
				while (!in_array($floorId,$floors) && !in_array(((string)$node['id']), $lasts) && !empty($floorId) && !empty($node)) {
				  ...
				  echo date("H:i:s") . " floor=$floorId $cnt row $nodeId\n";
				  $cnt++;
				  ...
				  echo "\n" . date("H:i:s") . " floor=$floorId $cnt row $nodeId DONE\n";
				  $lasts[] = $nodeId;
				  $floors[] = $floorId;
				  ...
				  $paths[$nodeId] = $path;
				  $str .= $path['path'];
			          $node = $xml->xpath("//floor[@id='" . $floorId . "']/planables/planable[@id='" . $path['end'] . "']")[0];
				  $node = $xml->xpath("//floor[@id='" . $path['floor'] . "']/planables/planable[@type='item']/item[@id='" . $node->item['id'] . "']")[0]->xpath( 'parent::*' )[0];
				  $floorId = $path['floor'];
...
function findPath($db, $oldPaths, $allFloors, $accesses, $start, $floor, $floorDest, $floors, $xml, $last=array(), $distance=0, $type="shortest", $end=0) {
				...
				if ($endId == $end) {
				  ...
				  $type = (string)$check['type'];
			     	   $x = $check->position['x'];
			     	   $y = $check->position['y'];
					echo date("Y-m-d H:i:s") . " find check $endId $type ";
				  if ($type == 'point') {
					  ...
					  $itemId = $check->item['id'];
					   if (in_array($itemId, $accesses)) {
						...
						$possibles = array();
						foreach($access as $a) {
							$aFloorId = (string)$a->xpath( 'parent::*' )[0]->xpath( 'parent::*' )[0]->xpath('parent::*')[0]['id'];
	     	   ...
	     	   echo date("Y-m-d H:i:s") . " find check $startId $type ";
				  ...
				  if ($type == 'point') {
					  ...
					  $itemId = $check->item['id'];
					   if (in_array($itemId, $accesses)) {
						...
						$possibles = array();
						foreach($access as $a) {
							$aFloorId = $a->xpath( 'parent::*' )[0]->xpath( 'parent::*' )[0]->xpath('parent::*')[0]['id'];
							if (!empty($aFloorId) && $floor != $aFloorId) {
								$found = true;
